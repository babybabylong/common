...
func TestOssFilePath_UnmarshalText(t *testing.T) {
	SetOssFromHost(testFromHost)
	SetOssToHost(testToHost)

	f := newTestOssFile(1, testOssFilePath, "测试")
	...
	d1 := `{"id":1,"link":"https://d.khols8.com//first/icon/contract.png","name":"测试"}`

	var f2 = &testOssFile{}
	require.NoError(t, json.Unmarshal([]byte(d1), f2))
