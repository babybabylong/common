...
*	error  	error          	错误
*/
func (m *client) Summarize(ownerID int64, amount decimal.Decimal) error {
	var (
		slotValue int64
		err       error
	)
...
func (m *client) SummarizeDay(date, ownerID int64, amount decimal.Decimal) error {
	// 写入或者更新
	slotValue := date
...
func (m client) getSlotValue(userID int64) (value int64, err error) {
	switch m.slot {
	case SlotDay: // 如果是天,那么就是yyyy-mm-dd
		return int64(helpers.GetDate()), nil
	case SlotWhole:
		return userID, nil
	default:
		return -1, newErrNotSupportSlot(m.slot)
	}
}
